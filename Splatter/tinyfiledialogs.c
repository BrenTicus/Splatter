...
v1.12.2 [April 23, 2015] zlib licence.
...
\nor\tXdialog\nor\tpython 2 with tkinter\nor\tdialog (& xterm if a console is missing)\
\nor\ta console for basic input" ;
...
static char const * terminalName ( )
{
    ...
    if ( lTerminalName[0] == '*' )
    {
		if ( isDarwin() && strcpy(lTerminalName , "/opt/X11/bin/xterm" )
...
{
	return !( tinyfd_forceConsole && (isatty(1) || dialogName()) )
			&& getenv ( "DISPLAY" );
}
...
if res==False :\n\tprint 0\n\
			 ...
			 && !strcmp("ok" , aDialogType) )
	{
		...
		strcat ( lDialogString , "\"" ) ;
		  ...
		  && ! strcmp("ok" , aDialogType) && xmessagePresent() )
	{
